...
public class FrequentReuseDistanceFinder {
	MongoClient mongoClient;
	MongoDatabase db;
	MongoCollection<Document> docs;
	String collection;
	
	public FrequentReuseDistanceFinder(String collection) {
	...
	private void CheckDocument(Document doc) {
		...
		if(reuse_distance > -1) {
			...
			match.append("$match", new Document("time", new Document("$gte", time).append("$lt", reused))
					.append("reuse_distance", new Document("$gte", 1)));
			...
			doc.append("frequent_reuse_distance", new Long(size));
			
			docs.updateOne(new Document("time", time),
			        new Document("$set", doc));
		} else {
			doc.append("frequent_reuse_distance", new Long(-1));
